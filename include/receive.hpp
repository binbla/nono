...
class Receiver {
   ...
   public:
    ...
    // 替换 receive 层配置。
    // 注意：load_monitor 是非拥有指针，调用方要保证其生命周期覆盖 Receiver
    // 使用期。
    ...
    // UDP 回调中最常用的入口。
    // 它负责识别消息类型、parse、mac 验证、必要时发 CookieReply，并调用
    // protocol。 plaintext_out 用于承接 transport 明文；结果中的 plaintext_size
    // 表示有效长度。
    ...
    // 消费 initiation。
    // 成功时返回 ConsumedInitiation 并带出 peer；高负载且 mac2 无效时会发送
    // CookieReply。
